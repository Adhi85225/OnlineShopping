...
<div class="container border login-wrapper py-3  m-8" style="width:60%"; >
  <form class="" action="deliverylogin.php" method="post" >

    <center>
      <h3 class="login-header"> Delivery Login </h3>
      <hr>
...
<p>Don't have account? <a href="deliverysignup.php">Click here</a></p>
...
if(isset($_POST['login'])){
  ...
  $query="select *  from deliveryboy where email='$email' and password='$password' ";
  ...
  if(mysqli_num_rows($query_solution)>0)
  {
    ...
    echo "<script>alert('user not found try to signup')</script>";
      echo "<script>window.location='deliverysignup.php'</script>";
  }
}
